...
以下のコンテキストを使用して、質問に答えてください。500文字以内にわかりやすくまとめてください。適切な箇所に改行を入れてください。もしコンテキストに答えがない場合は、「わかりません」と答えてください。
...
export async function POST(request: Request) {
  console.log("チャット処理開始");
    ...
    const vectorStore = new SupabaseVectorStore(embeddings, {
        client: supabase,
        tableName: 'manual_documents',
        queryName: 'match_manual_documents', // 作成した関数名
    });
    ...
    try {
      ...
      console.log("similaritySearch実行");
        ...
        const relevantDocs = await vectorStore.similaritySearch(question, 5); // 上位5件を取得
