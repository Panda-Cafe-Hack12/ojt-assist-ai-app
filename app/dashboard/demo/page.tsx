...
import { FileText, Search, Calendar, CheckCircle2, BarChart2, Settings, X } from "lucide-react"
...
export default function DashboardPage() {
  ...
  const ojtData = {
    ...
    daysRemaining: 28,
    learnFiles: [
      { fileName: "ファイル1", goalLevel: 2, viewCount: 3, totalPages: 5, currentPage: 5 },
      { fileName: "ファイル2", goalLevel: 3, viewCount: 15, totalPages: 36, currentPage: 24 },
      { fileName: "ファイル3", goalLevel: 1, viewCount: 3, totalPages: 5, currentPage: 3 },
      { fileName: "ファイル4", goalLevel: 4, viewCount: 20, totalPages: 45, currentPage: 34 },
      { fileName: "ファイル5", goalLevel: 5, viewCount: 25, totalPages: 80, currentPage: 49 },
    ],
    totalReadPages: 95,
    totalPages: 125,
    completionRate: 75,
  ...
  const totalSearches = filteredCategories.reduce((sum, item) => sum + item.searchCount, 0);
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">残り {ojtData.daysRemaining} 日</p>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="flex flex-row items-center justify-between pb-2">
            ...
            <CardContent>
              <div className="text-2xl font-bold">{ojtData.completionRate}%</div>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              <CardTitle className="text-sm font-medium">ファイル閲覧</CardTitle>
              <FileText className="h-4 w-4 text-muted-foreground" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold">
                {ojtData.learnFiles.reduce((sum, file) => sum + file.viewCount, 0)}
              </div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{searchData.reduce((sum, item) => sum + item.searchCount, 0)}</div>
                  ...
                  <div className="mt-6 border-t pt-4">
                    ...
                    {searchTerm && filteredCategories.length > 0 && (
                      <div className="mt-2 text-xs text-muted-foreground">
                        <span className="font-medium">{filteredCategories.length}</span> 件の結果が見つかりました
                      </div>
                    )}
                  </div>
                  <div className="mt-6">
